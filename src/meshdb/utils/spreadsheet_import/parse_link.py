...
def load_links_supplement_with_uisp(spreadsheet_links: List[SpreadsheetLink]):
    ...
    for uisp_link in uisp_links:
        ...
        if len(
            ...
            logging.warning(f"Skipping duplicate link between {from_device} and {to_device}")
            continue
    ...
    for spreadsheet_link in spreadsheet_links:
        # FIXME: This part does not appear to be working. We don't have any spreadsheet data
        try:
