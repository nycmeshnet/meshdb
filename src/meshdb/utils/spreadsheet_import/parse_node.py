...
def get_map_display(notes: str) -> Node.MapDisplay.choices:
    ...
    if is_supernode:
        return Node.MapDisplay.SUPERNODE
    if is_pop:
        return Node.MapDisplay.POP
    if is_ap:
        return Node.MapDisplay.AP
    if is_hub and not_potential_hub:
        return Node.MapDisplay.HUB
    if is_remote:
        return Node.MapDisplay.REMOTE

    return Node.MapDisplay.NODE
...
def get_or_create_node(
    ...
    return Node(
        ...
        map_display=get_map_display(row.notes) if row.notes else Node.MapDisplay.NODE,
