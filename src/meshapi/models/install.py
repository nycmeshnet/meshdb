...
class Install(models.Model):
    ...
    # We applied a custom migration to mark this column as "GENERATED BY DEFAULT AS IDENTITY"
    # in the SQL DDL which causes this field to autopopulate at the DB level
    install_number = models.IntegerField(unique=True, null=False, editable=False)
    ...
    def _save_table(self, *args: Any, **kwargs: Any) -> Any:
        result = super()._save_table(*args, **kwargs)  # type: ignore[misc]

        # If this is a new object instance, we won't have a copy of the
        # install_number column because this is generated by postgres
        # on our behalf, so we need to sync with postgres right away, in
        # case the caller (or post_save hook) is about to read .install_number from us
        if self._state.adding:
            self.refresh_from_db()

        return result

    def _do_insert(self, manager: Any, using: Any, fields: Sequence[Field], returning_fields: Any, raw: Any) -> Any:
        # If we are creating a new object (implied here because this
        # func is only called on INSERT operations), and the install_number has
        # not been set explicitly, leave it out of our SQL query, which will
        # instruct Postgres to automatically populate it from the identity sequence
        if self.install_number is None:
            fields = {f for f in fields if f.name != "install_number"}

        return super()._do_insert(manager, using, fields, returning_fields, raw)  # type: ignore[misc]
