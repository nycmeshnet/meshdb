...
locals {
  ...
  command = "for i in {0..12}; do echo install packages...; sleep 10; sudo apt-get -o DPkg::Lock::Timeout=180 update && sudo apt-get -o DPkg::Lock::Timeout=180 install -y ${local.packages} && break; done"
}
